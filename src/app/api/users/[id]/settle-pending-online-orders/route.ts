 ...
 export async function POST(
   ...
   try {
     ...
     if (!Array.isArray(batchIds) || batchIds.length === 0) {
       return new NextResponse(JSON.stringify({ error: "No batches provided" }), { status: 400 });
     }
     ...
     const result = await db.$transaction(async (tx) => {
       ...
       if (batches.length !== batchIds.length) {
         throw new Error("Some selected batches are not pending or not accessible");
       }

       const totalToDeduct = batches.reduce((sum, b) => sum + (b.totalAmount || 0), 0);
       ...
       return { success: true, deducted: totalToDeduct, remainingBalance: updatedUser?.balance || 0 };
     });
