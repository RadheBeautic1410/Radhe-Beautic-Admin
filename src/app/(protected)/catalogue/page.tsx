...
const ListPage = () => {
  ...
  const fetchUpdatedData = async () => {
    ...
    console.log(
      new Date(storedDelTime2).getTime(),
      new Date(tempRes.data.time).getTime(),
    ...
    if (
      ...
      localStorage.setItem(
        "radhe-time-1",
        JSON.stringify(currTime.toISOString())
      );
    ...
    const fetchData = async () => {
      try {
        ...
        for (let i = 0; i < allKurtiData.length; i++) {
          for (let j = 0; j < allCategoryName.length; j++) {
            if (
              allKurtiData[i].category.toLowerCase() ===
                allCategoryName[j].name.toLowerCase() &&
              ...
              if (allCategoryName[j].sellingPrice === 0) {
                allCategoryName[j].sellingPrice = parseInt(
                  allKurtiData[i].sellingPrice || "0"
                );
              }
              // allCategoryName[j].sellingPrice = Math.max(parseInt(allKurtiData[i].sellingPrice || "0"), allCategoryName[j].sellingPrice);
              allCategoryName[j].actualPrice +=
                cnt * parseInt(allKurtiData[i].actualPrice || "0");
              sum2 += cnt;
        ...
        console.log(allCategoryName);
        const sortedCategory = allCategoryName.sort(
          (a: category, b: category) => b.sellingPrice - a.sellingPrice
        );
        setTotalItems(sum1);
        setTotalPiece(sum2);
              ...
              for (let i = 0; i < allKurtiData.length; i++) {
                for (let j = 0; j < allCategoryName.length; j++) {
                  if (
                    ...
                    for (
                      let k = 0;
                      k < allKurtiData[i].sizes.length || 0;
                      k++
                    ) {
                      cnt += allKurtiData[i].sizes[k].quantity || 0;
                    }
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {displayCategoryData.map((cat, idx) => (
                          <TableRow key={`${idx}-1`}>
                            <TableCell key={idx * idx} className="text-center">
                              {idx + 1}
                            </TableCell>
                            <TableCell
                              key={cat.name}
