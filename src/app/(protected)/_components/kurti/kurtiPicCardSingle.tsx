...
const KurtiPicCardSingle: React.FC<KurtiPicCardSingleProps> = ({
  data,
  idx,
  onPicDelete,
  onImageToggle,
  ...
  const loadWatermark = async (rightText: string, leftText: string) => {
    const imgDom = document.querySelector(
      `#download${data.code}-${idx}`
    ) as HTMLImageElement;
    const imgDom2 = document.querySelector(
      `#download${data.code}-${idx}`
    ) as HTMLImageElement;
  ...
  const handleDelete = async () => {
    try {
      ...
      const res = await fetch(
        `/api/kurti/deletePic?code=${data?.code}&idx=${idx}`
      );
  ...
  const handleImageVisibilityToggle = async (
    imageId: string,
  ...
  return (
    <div id="container" className="p-3 bg-slate-300">
      <img
        className={` ${data.images[idx].is_hidden ? "opacity-50" : ""}`}
        id={`${data.code}${idx}-visible`}
        src={data.images[idx].url}
        crossOrigin="anonymous"
        width={"300px"}
        height={"300px"}
        alt=""
