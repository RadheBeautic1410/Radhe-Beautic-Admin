...
const ReadyOrders = () => {
  ...
  const fetchReadyOrders = async (
    ...
    const res = await fetch("/api/orders/getOrderslist", {
      method: "POST",
      body: JSON.stringify({
        status: "PROCESSING",
        pageNum: pageParam,
  ...
  const handleSearch = async (searchId: string) => {
    ...
    if (searchId.length !== 13) {
      ...
      queryClient.invalidateQueries({
        predicate: (query) => query.queryKey[0] === "readyOrders",
      });
    } else {
      await searchOrderQuery.refetch();
      if (
        searchOrderQuery.isSuccess &&
        searchOrderQuery.data &&
        searchOrderQuery.data.success
      ) {
        // If the order is found, update the local state to show this order
        queryClient.setQueryData(
          ["pendingOrders", page, pageSize, dateRange],
          (old: any) => ({
            ...old,
            data: {
              ...old.data,
        ...
        searchOrderQuery.isError ||
        (searchOrderQuery.data && !searchOrderQuery.data.success)
      ) {
      ...
      cell: ({ row }) => {
        return (
          <>
            ...
            <div className="font-semibold text-gray-800">
              {row.original.cart.CartProduct.reduce(
                (sum, product) =>
                  sum +
  ...
  if (isFetching || searchOrderQuery.isFetching) {
    return <DataLoader loading={isFetching || searchOrderQuery.isFetching} />;
  }
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2 py-4">
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          >
            <SelectTrigger className="w-[180px]">{`${pageSize}`}</SelectTrigger>
            <SelectContent>
