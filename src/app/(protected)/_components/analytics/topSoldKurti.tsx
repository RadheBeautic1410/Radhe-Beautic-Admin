...
const TopSoldKurti = () => {
  ...
  const [filterType, setFilterType] = useState<"ALL" | "YEAR" | "MONTH" | "DATE">("ALL");
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  const fetchFilteredSales = async (
    date: Date | undefined,
    filter: "ALL" | "YEAR" | "MONTH" | "DATE"
  ) => {
  ...
  const { isPending, isFetching, data } = useQuery({
    queryKey: ["filteredSales", date, filterType],
    queryFn: () => fetchFilteredSales(date, filterType),
  ...
  const filteredSalesList = useMemo(() => {
    const sortedList = [...(data?.salesList || [])]
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-6 space-y-6">
      <h2 className="text-2xl font-semibold mb-4">Top 100 Sold Kurti</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="w-full sm:w-[40%]">
          ...
          >
            <option value="ALL">Top 100 Overall</option>
        ...
        {(filterType !== "ALL") && (
          <div className="w-full sm:w-[40%]">
            <label className="block text-sm font-medium text-muted-foreground mb-2">
              Select {filterType === "YEAR" ? "Year" : filterType === "MONTH" ? "Month" : "Date"}
            </label>
            <Popover>
